...
2. Перейдите в папку ПК клиента:
   ...
   cd gamedesk_pc
   ```
3. **Для запуска в режиме разработчика** используйте скрипт `run_dev.bat` (он автоматически создаст виртуальное окружение, активирует его и установит все необходимые зависимости из `requirements.txt`, не засоряя систему):
   ```cmd
   run_dev.bat
   ```
4. **Для сборки автономного .exe файла** используйте скрипт `build_exe.bat`. Он также работает в изолированном окружении и соберет готовое к использованию приложение в папку `dist/GameDesk`. Вы сможете перенести эту папку на любой ПК, даже без установленного Python.
   ```cmd
   build_exe.bat
   ```
5. **Настройка телеметрии (HWiNFO64 и FPS):**
   - Основным и рекомендуемым источником данных о ПК (температуры CPU, GPU и т.д.) является программа **HWiNFO64**.
   - В корне репозитория есть установщик `hwi64_848.exe`. Установите и запустите эту программу.
   - В настройках HWiNFO64 (Settings) обязательно нужно поставить галочку напротив **"Shared Memory Support"** (Поддержка общей памяти).
   - *Важно: в бесплатной версии программы эта галочка автоматически сбрасывается каждые 12-24 часа, поэтому её нужно периодически включать заново.*
   - Если HWiNFO64 недоступна, скрипт попытается использовать *LibreHardwareMonitor* как запасной вариант (однако через него не будет отображаться температура CPU).

   **Как включить отображение FPS на экране:**
   Для вывода счетчика FPS вам понадобится утилита **RTSS (RivaTuner Statistics Server)**, которая часто устанавливается вместе с MSI Afterburner.
   - Скачайте и установите RTSS с [официального сайта Guru3D](https://www.guru3d.com/download/rtss-rivatuner-statistics-server-download/).
   - Запустите RTSS и убедитесь, что он работает (иконка в трее Windows).
   - Запустите HWiNFO64 и перейдите в его настройки. На вкладке "OSD (RTSS)" (или "Custom" / "HWiNFO Shared Memory") должна стоять галочка "Shared Memory Support", как описано выше.
   - HWiNFO64 автоматически начнет получать данные от RTSS, и в его списке сенсоров появится раздел **"RTSS"** с показателем **"Framerate"**.
   - Всё готово! Приложение GameDeskPC само найдет этот сенсор и начнёт транслировать FPS на ваш экран ESP32.
